...
echo "=== Releasing scout v$VERSION ==="
...
if ! git diff --cached --quiet; then
    git commit -m "release: scout $VERSION"
else
...
git tag -a "v$VERSION" -m "scout v$VERSION"
...
if command -v gh &> /dev/null; then
    ...
    if [ ${#ASSETS[@]} -gt 0 ]; then
        gh_args=("release" "create" "v$VERSION" "${ASSETS[@]}" "--title" "scout v$VERSION" "--generate-notes")
...
echo "=== Release scout v$VERSION completed successfully! ==="
